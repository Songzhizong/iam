...
public final class GoogleAuthenticator implements IGoogleAuthenticator {
     ...
     */
    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(GoogleAuthenticator.class);
    ...
    private String calculateSecretKey(byte[] secretKey) {
        return switch (config.getKeyRepresentation()) {
            case BASE32 -> Base32.encode(secretKey);
            case BASE64 -> Base64.getEncoder().encodeToString(secretKey);
